#ifndef SYSTEM_CONFIG_H
#define SYSTEM_CONFIG_H

#include <Arduino.h>

// =============================================================================
// SWARM SYSTEM PARAMETERS & CONFIGURATION DEFINITIONS
// =============================================================================

// --- Robot Kinematics Configurations ---
const float WHEEL_DIAMETER_METERS   = 0.065f;   // 65 mm wheel diameter
const float WHEEL_RADIUS_METERS     = WHEEL_DIAMETER_METERS / 2.0f;
const float AXLE_TRACK_METERS       = 0.145f;   // 145 mm track width (distance between wheels)
const float ENCODER_TICKS_PER_REV   = 4096.0f;  // 12-bit magnetic AS5600 resolution

// --- Real-time Loop Rates & Core Pinning Configurations (FreeRTOS) ---
const TickType_t CONTROL_LOOP_PERIOD_MS = pdMS_TO_TICKS(10);  // 100Hz real-time motor control (Core 1)
const TickType_t SENSOR_READ_PERIOD_MS  = pdMS_TO_TICKS(20);  // 50Hz sensor read (Core 1)
const TickType_t TELEMETRY_PERIOD_MS    = pdMS_TO_TICKS(100); // 10Hz network broadcasting (Core 0)

// --- TCA9548A I2C Channel Assignments ---
const uint8_t MUX_I2C_ADDR              = 0x70; // Hardwired on board
const uint8_t MUX_CHAN_LEFT_ENCODER     = 0;
const uint8_t MUX_CHAN_RIGHT_ENCODER    = 1;
const uint8_t MUX_CHAN_SWEEP_ENCODER    = 2;

// --- IMU Native Configuration Constraints ---
const uint8_t IMU_I2C_ADDR              = 0x69; // Hardwired address

// --- PID Speed Controller Parameters ---
const float PID_KP                      = 2.45f;
const float PID_KI                      = 0.12f;
const float PID_KD                      = 0.05f;

#endif // SYSTEM_CONFIG_H
